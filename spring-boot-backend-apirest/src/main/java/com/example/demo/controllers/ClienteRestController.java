...
public class ClienteRestController {
	...
	public ResponseEntity<?> show(@PathVariable Long id) {
		...
		//Manejo de errores en bbdd
		try {
			cliente = clienteService.findById(id);	
		} catch(DataAccessException e) {
			response.put("mensaje","Error al realizar consulta en BBDD");
			response.put("error",e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}
		
		
		//Manejo de causisticas que generarian error
		if(cliente == null) {
	...
	public ResponseEntity<?>  update(@RequestBody Cliente cliente, @PathVariable Long id) {
		...
		if(clienteActual == null) {
			response.put("mensaje","Error: no se puede editar, l cliente ID; ".concat(id.toString().concat(" no existe en la BBDD")));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.NOT_FOUND);
		}
